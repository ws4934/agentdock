...
补充检查覆盖“最后一次条件样本返回的同时发生取消/本机停止”：`desktop_wait` 在形成结果前重新核验取消和会话代次，防止将其误写成 met 或 timeout。相应四种竞态回归由 `wait_completion_test.go` 覆盖。新增 Python 故障脚本也已登记到 `scripts/governance/inventory.yaml`；以纳入 Git 跟踪后的全仓库治理门禁为准，而非只检查未跟踪工作区。
