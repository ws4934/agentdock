...
这是**一个 Core 进程内按私有能力令牌独占的桌面任务**，不是按模型 turn 自动识别客户端身份的系统。正式宿主要求 `desktop_task begin`，任务内的应用/前后台范围由本机用户批准，详见 [computer-use-reliability.md](computer-use-reliability.md)。停止会阻断后续桌面输入和目标窗口观察，直到本机用户恢复；终端、浏览器专用工具、模型推理等不在此停止范围内。状态不跨进程持久化，也不是新的系统安全沙箱。
...
| 执行下一步 | 允许新的观察/等待和至多一个改变状态的桌面操作，随后自动暂停；不重放旧输入。 |
| 应用授权 | 未信任应用/模式显示身份卡片，可本次允许、始终允许或拒绝；持久许可可在菜单中撤销。 |
| 输入清理失败 | 保留人工检查提示，不将释放失败显示为正常 stopped；人工确认后仍停在 stopped，恢复另行操作。 |
| 菜单栏 | 可重新显示窗口、暂停/恢复或停止。用户停止后，菜单栏仍保留明确的本机恢复入口。 |
| 等待下一步 | 表示没有正在执行的桌面工具；不伪装成“模型思考”。60 秒无桌面活动后结束视频/操作会话，但不自动释放独占任务。 |
...
- `computeruse.command`：包含当前 `session_id`，支持 `pause`、`stop`、`resume`、`step`、`acknowledge_cleanup`、`approve_application`、`deny_application`、`end_task`。应用批准/拒绝额外携带当前 `approval_id`。拒绝过期会话、其他活动控制器和未知参数。

停止/暂停意图通过心跳的 `stop_session_id` / `pause_session_id` 保留到确认，核心先应用意图再续租；连接恢复不会丢失用户的停止意图。Timer 与回包运行在主 RunLoop 的 common/eventTracking/modalPanel 模式，菜单跟踪不会被误判为断线。不能仅修改 Timer 模式而把回调仍交给嵌套菜单期间可能延迟的 MainActor Task。

释放失败进入 `cleanup_failed`，普通恢复被禁止；用户明确检查并确认人工释放后仍处于 stopped。预览瞬时失败对同一目标最多尝试4次并退避，用户停止/拒绝/权限失败不会自动重启，必须显式本机重试。菜单栏附带最近6条操作元数据（核心最多32条），不保存输入或图片；紧急停止组合为 Control-Option-Command-F12，注册冲突时保留普通停止入口。
