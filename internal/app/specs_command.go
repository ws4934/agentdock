...
func commandToolSpecs() []ToolSpec {
	return []ToolSpec{
		...
		{Name: "job_control", Contract: commandToolContract, Title: "Control durable jobs", Description: "Cancel, archive a confirmed terminal job, or explicitly abandon an unknown execution after verifying its owned process group ended. Abandonment releases capacity but never retries or certifies success. Cancellation requires a terminal receipt.", Annotations: mutatingToolAnnotations(true, false), Handler: typedToolHandler("job_control", func(ctx context.Context, r *Runtime, request toolcommand.JobControlRequest) (Result, error) {
