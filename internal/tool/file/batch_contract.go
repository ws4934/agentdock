...
func batchInput(name string) (map[string]any, bool) {
	...
	switch name {
	case "read_files":
		item := toolcontract.InputObject(map[string]any{"cursor": map[string]any{"type": "string", "maxLength": 1024, "description": "Exact next_cursor from a preceding block; omit line selection."}, "path": str("Native Host path or Skill resource."), "start_line": integer("Inclusive start, default 1.", 1, 2147483200), "end_line": integer("Inclusive end; at most 400 lines per request. Default start+199.", 1, 2147483647), "expected_read_revision": str("Expected full-file revision from an earlier read.")}, "path")
		toolcontract.ForbidTogether(item, "cursor", "start_line")
		toolcontract.ForbidTogether(item, "cursor", "end_line")
